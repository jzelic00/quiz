...
<form id="form1" action="index.php?action=nova_igra" method="POST">	
				<?php
						...
						PostaviPitanje();
							...
							<div id="obican_box2">
								...
								<div class="naslov">
									...
									<div class="gumbici">
										<button type="button" class="butt_new" name="'.$imePitanja. '" value="' . $odgovor1 . '" onclick="myFunction(value)"> '. $odgovor1 . '</button><br>
										<button type="button" class="butt_new" name="'.$imePitanja. '" value="' . $odgovor2 . '" onclick="myFunction(value)"> '. $odgovor2 . '</button><br>
										<button type="button" class="butt_new" name="'.$imePitanja. '" value="' . $odgovor3 . '" onclick="myFunction(value)"> '. $odgovor3 . '</button><br>
										<button type="button" class="butt_new" name="'.$imePitanja. '" value="' . $odgovor4 . '" onclick="myFunction(value)"> '. $odgovor4 . '</button><br>
									</div>
							</div>
					...
					$_SESSION['nizTocnihOdgovora'] = $nizTocnihOdgovora;					
				?>
<div id="div4">
	<div id="obican_box2">
		<div id="obican_box_naslov2">
		...
		<div class="gumbici">
			<p>BROJ BODOVA: <span id="ostvarniBod"></span></p>
			<p>BROJ TOCNIH ODGOVORA: <span id="tocniOdgovori"></span></p>
		</div>
	</div>	
</div>	
	...
	function myFunction(vrijednost) {
		...
		if(i==(brPitanja - 1)) {
			var xmlhttp = new XMLHttpRequest();
       		xmlhttp.onreadystatechange = function() {
	            if (this.readyState == 4 && this.status == 200) {
	            	//bodovi=this.responseText;

	            	//window.alert(this.responseText);
	            	vraceno = this.responseText;
	            	ss = vraceno.split(",");
        	...
        	xmlhttp.open("GET", "calculate.php?korisnik="+nizOdgovora, false);
       		xmlhttp.send();
       		document.getElementById('ostvarniBod').innerHTML = ss[0];
       		document.getElementById('tocniOdgovori').innerHTML = ss[1];
